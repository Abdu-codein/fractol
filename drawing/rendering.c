...
/*   Updated: 2026/06/06 06:31:40 by amardini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (info->colour == 1)
		return(pixel_draw(info, x, y, dynamic_colour(iter)));
	if (info->colour == 2)
...
void	pixel_draw(t_fractal *info, int x, int y, int colour)
{
	char *dst;
	dst = info->addr + (y * data->line_len + x * (data->bpp /8));
